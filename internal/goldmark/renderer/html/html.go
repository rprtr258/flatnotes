...
func (b EastAsianLineBreaks) softLineBreak(thisLastRune, siblingFirstRune rune) bool {
	switch b {
...
func eastAsianLineBreaksCSS3DraftSoftLineBreak(thisLastRune, siblingFirstRune rune) bool {
	...
	if fun.Contains(thisLastRuneEastAsianWidth, "F", "W", "H") &&
		fun.Contains(siblingFirstRuneEastAsianWidth, "F", "W", "H") {
...
func (r *Renderer) writeLines(w util.BufWriter, source []byte, n ast.Node) {
	for _, line := range n.Lines().Values {
...
func (r *Renderer) renderRawHTML(
	...
	if r.Unsafe {
		n := node.(*ast.RawHTML)
		for _, segment := range n.Segments.Values {
