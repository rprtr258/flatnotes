...
type Config struct {
	...
	NodeRenderers util.PrioritizedSlice[NodeRenderer]
}
...
type withNodeRenderers struct {
	value []util.PrioritizedValue[NodeRenderer]
}
...
func WithNodeRenderers(ps ...util.PrioritizedValue[NodeRenderer]) Option {
...
func (o *withOption[T]) SetConfig(c *Config) {
	c.Options[o.name] = o.value
}

// WithOption is a functional option that allow you to set
// an arbitrary option to the parser.
func WithOption[T any](name OptionName, value T) Option {
	return &withOption[T]{name, value}
}
...
func (r *renderer) Register(kind ast.NodeKind, v NodeRendererFunc) {
	...
	r.maxKind = max(r.maxKind, int(kind))
}
...
func (r *renderer) Render(w io.Writer, source []byte, n ast.Node) error {
	...
	err := ast.Walk(n, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
		if f := r.nodeRendererFuncs[n.Kind()]; f != nil {
			return f(writer, source, n, entering)
		}
