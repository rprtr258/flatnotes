...
type Segment struct {
	Start   int // Start is a start position of the segment.
	Stop    int // Stop is a stop position of the segment. This value should be excluded.
	Padding int // Padding is a padding length of the segment.
}
...
func (t Segment) Value(buffer []byte) []byte {
	...
	return append(bytes.Repeat(space, t.Padding), buffer[t.Start:t.Stop]...)
}
...
func (t *Segment) TrimLeftSpace(buffer []byte) Segment {
	l := util.TrimLeftSpaceLength(buffer[t.Start:t.Stop])
...
// Append appends all elements of given segments after the tail of the collection.
