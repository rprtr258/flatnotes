...
type FindClosureOptions struct {
	// CodeSpan is a flag for the FindClosure.
	// If this is set to true, FindClosure ignores closers in codespans.
	CodeSpan bool

	// Nesting is a flag for the FindClosure.
	// If this is set to true, FindClosure allows nesting.
	Nesting bool

	// Newline is a flag for the FindClosure.
	// If this is set to true, FindClosure searches for a closer over multiple lines.
	Newline bool

	// Advance is a flag for the FindClosure.
	// If this is set to true, FindClosure advances pointers when closer is found.
	Advance bool
}
...
func (r *blockReader) ResetPosition() {
	...
	if r.segmentsLength > 0 {
		last := r.segments.Values[r.segmentsLength-1]
		r.last = last.Stop
	}
	r.AdvanceLine()
}
...
func (r *blockReader) Reset(segments *Segments) {
	r.segments = segments
	r.segmentsLength = len(segments.Values)
	r.ResetPosition()
}
...
func (r *blockReader) Value(seg Segment) []byte {
	...
	for ; line >= 0; line-- {
		if seg.Start >= r.segments.Values[line].Start {
			break
		}
	}
	i := seg.Start
	for ; line < r.segmentsLength; line++ {
		s := r.segments.Values[line]
		if i < 0 {
			i = s.Start
		}
...
func (r *blockReader) PrecendingCharacter() rune {
	...
	if len(r.segments.Values) < 1 {
		return rune('\n')
	}
	firstSegment := r.segments.Values[0]
