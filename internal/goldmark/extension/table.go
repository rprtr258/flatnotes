...
import (
	...
	gast "github.com/rprtr258/flatnotes/internal/goldmark/ast"
	"github.com/rprtr258/flatnotes/internal/goldmark/extension/ast"
...
func (b *tableParagraphTransformer) Transform(node *gast.Paragraph, reader text.Reader, pc parser.Context) {
	lines := node.Lines()
	if len(lines.Values) < 2 {
		return
	}
	for i := 1; i < len(lines.Values); i++ {
		alignments := b.parseDelimiter(lines.Values[i], reader)
		if alignments == nil {
			continue
		}
		header := b.parseRow(lines.Values[i-1], alignments, true, reader, pc)
		...
		for j := i + 1; j < len(lines.Values); j++ {
			table.AppendChild(table, b.parseRow(lines.Values[j], alignments, false, reader, pc))
		}
		node.Lines().SetSliced(0, i-1)
		node.Parent().InsertAfter(node.Parent(), node, table)
		if len(node.Lines().Values) == 0 {
			node.Parent().RemoveChild(node.Parent(), node)
		} else {
			last := node.Lines().Values[i-2]
			last.Stop-- // trim last newline(\n)
			node.Lines().Values[i-2] = last
		}
	}
}
...
func (e *table) Extend(m *goldmark.Markdown) {
	m.Parser().AddOptions(
		...
		parser.WithASTTransformers(
			util.Prioritized[parser.ASTTransformer](defaultTableASTTransformer, 0),
		),
	)
