...
func (s *ids) Generate(value []byte, kind ast.NodeKind) []byte {
	...
	if _, ok := s.values[string(result)]; !ok {
...
func DefaultBlockParsers() []util.PrioritizedValue[BlockParser] {
	return []util.PrioritizedValue[BlockParser]{
...
//	RawHTMLParser, 400
//	EmphasisParser, 500
func DefaultInlineParsers() []util.PrioritizedValue[InlineParser] {
	return []util.PrioritizedValue[InlineParser]{
...
func DefaultParagraphTransformers() []util.PrioritizedValue[ParagraphTransformer] {
	return []util.PrioritizedValue[ParagraphTransformer]{
		util.Prioritized[ParagraphTransformer](LinkReferenceParagraphTransformer, 100),
	}
}
...
func (p *parser) addBlockParser(v util.PrioritizedValue[BlockParser], options map[OptionName]any) {
	bp := v.Value
	tcs := bp.Trigger()
...
func (p *parser) addInlineParser(v util.PrioritizedValue[InlineParser], options map[OptionName]any) {
	ip := v.Value
	tcs := ip.Trigger()
	so, ok := v.Value.(SetOptioner)
	if ok {
...
func (p *parser) addParagraphTransformer(v util.PrioritizedValue[ParagraphTransformer], options map[OptionName]any) {
	pt := v.Value
	so, ok := v.Value.(SetOptioner)
	if ok {
		for oname, ovalue := range options {
...
func (p *parser) addASTTransformer(v util.PrioritizedValue[ASTTransformer], options map[OptionName]any) {
	at := v.Value
	so, ok := v.Value.(SetOptioner)
	if ok {
		for oname, ovalue := range options {
	...
	retry:
		...
		if line[lineLength-1] == '\n' {
			...
			case ((lineLength >= 3 && line[lineLength-2] == '\\' &&
				line[lineLength-3] != '\\') || (lineLength == 2 && line[lineLength-2] == '\\')): // ends with \\n
				lineLength -= 2
				lineBreakFlags |= lineBreakHard | lineBreakVisible
			case ((lineLength >= 4 && line[lineLength-3] == '\\' && line[lineLength-2] == '\r' &&
				line[lineLength-4] != '\\') || (lineLength == 3 && line[lineLength-3] == '\\' && line[lineLength-2] == '\r')): // ends with \\r\n
				lineLength -= 3
				lineBreakFlags |= lineBreakHard | lineBreakVisible
			case lineLength >= 3 && line[lineLength-3] == ' ' && line[lineLength-2] == ' ': // ends with [space][space]\n
				lineLength -= 3
				lineBreakFlags |= lineBreakHard
			case lineLength >= 4 && line[lineLength-4] == ' ' && line[lineLength-3] == ' ' &&
				line[lineLength-2] == '\r': // ends with [space][space]\r\n
				lineLength -= 4
				lineBreakFlags |= lineBreakHard
			default:
