...
func generateAutoHeadingID(node *ast.Heading, reader text.Reader, pc Context) {
	var line []byte
	lastIndex := len(node.Lines().Values) - 1
	if lastIndex > -1 {
		line = node.Lines().Values[lastIndex].Value(reader.Source())
	}
...
func parseLastLineAttributes(node ast.Node, reader text.Reader, pc Context) {
	...
	lastLine := node.Lines().Values[lastIndex]
	...
	if ok && util.IsBlank(line[end.Start:]) {
		...
		node.Lines().Values[lastIndex] = lastLine
	}
}
